...
- `vendor/bin/sail composer ...` — all Composer commands
- `vendor/bin/sail npm run dev` / `build` — frontend dev/build
- `vendor/bin/sail artisan test --compact --filter=testName` — run Pest tests
- `vendor/bin/sail bin pint --dirty --format agent` — format PHP files
- `vendor/bin/sail phpstan analyse` — run static analysis
